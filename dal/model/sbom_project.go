...
type SBOMProject struct {
	...
	Inet          string    `json:"inet"             gorm:"column:inet;size:20"`      // 节点 IP
	Filepath      string    `json:"filepath"         gorm:"column:filepath;size:500"` // 文件路径
	SHA1          string    `json:"sha1"             gorm:"column:sha1;size:50"`      // 文件哈希
	...
	Exe           string    `json:"exe"              gorm:"column:exe;size:500"`      // 执行路径
	Username      string    `json:"username"         gorm:"column:username;size:100"` // 执行用户
