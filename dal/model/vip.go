...
type VIP struct {
	...
	VirtualIP   string    `json:"virtual_ip"   gorm:"column:virtual_ip;size:100"`          // 虚拟 IP
	VirtualPort int       `json:"virtual_port" gorm:"column:virtual_port"`                 // 虚拟 IP 开放的端口
	VirtualAddr string    `json:"virtual_addr" gorm:"column:virtual_addr;size:100"`        // 虚拟地址 virtual_ip + : + virtual_port
	Enable      bool      `json:"enable"       gorm:"column:enable;notnull;default:false"` // 是否开启
	IDC         string    `json:"idc"          gorm:"column:idc;size:20"`                  // IDC
	MemberIP    string    `json:"member_ip"    gorm:"column:member_ip;size:100"`           // 内部成员 IP
	MemberPort  int       `json:"member_port"  gorm:"column:member_port"`                  // 成员 端口
	Status      string    `json:"status"       gorm:"column:status;size:10"`               // 状态
	Priority    int       `json:"priority"     gorm:"column:priority"`                     // 优先级
	BizBranch   string    `json:"biz_branch"   gorm:"column:biz_branch;size:100"`          // 业务分支
	BizDept     string    `json:"biz_dept"     gorm:"column:biz_dept;size:100"`            // 业务部门
	BizType     string    `json:"biz_type"     gorm:"column:biz_type;size:100"`            // 业务类型
