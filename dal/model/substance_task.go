...
type SubstanceTask struct {
	...
	TaskID     int64     `json:"task_id,string"   gorm:"column:task_id;notnull"`
	MinionID   int64     `json:"minion_id,string" gorm:"column:minion_id;notnull;comment:节点 ID"`
	Inet       string    `json:"inet"             gorm:"column:inet;notnull;comment:节点 IP"`
	BrokerID   int64     `json:"broker_id,string" gorm:"column:broker_id;notnull;comment:节点所在的 broker_id"`
	BrokerName string    `json:"broker_name"      gorm:"column:broker_name;notnull;comment:节点所在的 broker 名字"`
	Failed     bool      `json:"failed"           gorm:"column:failed;notnull;default:false;comment:是否下发失败"`
	Reason     string    `json:"reason"           gorm:"column:reason;comment:失败原因"`
